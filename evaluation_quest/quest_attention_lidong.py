...
#添加代码 李东 4/2晚上
#transformers.models.llama.modeling_llama.py和transformers.models.qwen2.modeling_qwen2.py中的apply_rotary_pos_emb和 repeat_kv函数一致
...
def forward(
    ...
    **kwargs,
    ...
    if isinstance(past_key_value, DynamicCache):#假如使用DynamicCache类来存储past_key_value。即 past_key_value 是一个 DynamicCache 对象
        ...
        # print("kv_seq_len:",kv_seq_len)#Qwen2会输出0，longchat会输出1
...
global layer_id, layer_num_init
layer_id = 0
layer_num_init = True

model_name2layer_num={
    "lmsys/longchat-7b-v1.5-32k":32,
...
def enable_quest_attention_eval(model, args):
    global layer_id , layer_num_init
    if layer_num_init:
        if args.model_name_or_path not in model_name2layer_num:
            print(f"error: KEY model_name_or_path \"{args.model_name_or_path}\" not found in DICT \"model_name2layer_num\"")
            raise ValueError(f"KEY model_name_or_path \"{args.model_name_or_path}\" not found in DICT \"model_name2layer_num\"")
        layer_id = model_name2layer_num[args.model_name_or_path]#要求args.model_name_or_path要求全称
        layer_num_init=False
    ...
    for name, module in reversed(model._modules.items()):#先从离用户远的子模块（layer_id=31）开始，【假设整个model是个树，用户在左，结点先右再左最后本身】
        ...
        if isinstance(module, (LlamaAttention, MistralAttention, Qwen2Attention)):#if isinstance(对象/变量a,类/类型或其元组B) 如果对象/变量a的类/类型等于B或为元组之一，则为True。
            # For longchat model
            layer_id -= 1
            if layer_id<0:
                print("error:layer_id<0")
                raise ValueError("layer_id<0, please check your model's layer number in model_name2layer_num")
            else:
